...
- The user try to submit the form
- The user interact with the field and move the focus (either clicking or using the keyboard) to a different element

If any of the fields are invalid, the form will not be submitted. 
The directives hides the default validation error tooltips generated by the browser so make sure you add a suitable error message for the final user.
...
It's possible to pass the rule name as second parameter to the $invalid helper to show an error message
only when that specific rule fails
