...
| [`x-validate`](#x-validate) | Validate html forms and fields. |
...
```html
<form x-data>
    <input x-ref="foo" name="test" x-validate="['required']">
    <span x-show="$invalid($refs.foo)">Invalid element</span>
    <input type="submit">
</form>
```

The error message becomes visible when one of the following conditions occurs:
- The user tries to submit the form
- The user interacts with the field and move the focus (either clicking or using the keyboard) to a different element

If any of the fields are invalid, the form will not be submitted. 
The directive hides the default validation error tooltip generated by the browser so make sure you add a suitable error message for the final user.

**Add a css class to an invalid input**
```html
<form x-data>
    <input x-ref="foo" name="test" x-validate="['required']" x-class="{'error': $invalid($refs.foo)}>
    <input type="submit">
</form>
```
...
By design, the initial validation waits for the user to leave the input element before kicking in.
After that point, the field is revalidated every time the user interacts with the input field.
If you prefer to start showing the error message as the user types from the first interaction, you can use the `.immediate` modifier

```html
<form x-data>
    <input x-ref="foo" name="test" x-validate.immediate="['required']">
    <span x-show="$invalid($refs.foo)">Invalid element</span>
    <input type="submit">
</form>
```

**Multiple rules with different error messages**

It's possible to pass the rule name as second parameter to the $invalid helper to show an error message only when that specific rule fails

**Example:**
```html
<form x-data>
    <input x-ref="foo" name="test" x-validate="['required', 'minlenght:1']">
    <span x-show="$invalid($refs.foo, 'required')">Required field</span>
    <span x-show="$invalid($refs.foo, 'minlenght')">Min lenght: 1</span>
    <input type="submit">
</form>
```
...
Note, by design all the predefined rules except required do not trigger a validation error for empty values. This makes easier to define optional field following certain rules. If a field is required, make sure you pass the required rule at the beginning of the rule list.
...
Along with the predefined rules, user can define their own rules adding a callback to the window.AlpineValidationRules array. The input value will pe passed as first parameter. It's advised to always pass the test when the value is empty so the custom test will be consistent with the rest of  the library

**Example:**
```html
<script type="text/javascript">
    window.AlpineValidationRules['creditCardDate'] = (value) => value === '' || /^(0[1-9]|1[0-2])\/?([0-9]{2})$/.test(value)
</script>

<form x-data>
    <input x-ref="foo" name="test" x-validate="['required', 'creditCardDate']">
    <span x-show="$invalid($refs.foo, 'required')">Required field</span>
    <span x-show="$invalid($refs.foo, 'creditCardDate')">Invalid  credit  card date</span>
    <input type="submit">
</form>
```
It's also possible to accept a value as second parameter.

**Example:**
```html
<script type="text/javascript">
    window.AlpineValidationRules['between'] = (value, range) => {
        if (value === '') return true;
        let boundaries = range.split(',')
        return parseFloat(value) >= boundaries[0] && parseFloat(value) <= boundaries[1]
    }
</script>

<form x-data>
    <input x-ref="foo" name="test" x-validate="['required', 'between:10,20']">
    <span x-show="$invalid($refs.foo, 'required')">Required field</span>
    <span x-show="$invalid($refs.foo, 'between:10,20')">Value out of range</span>
    <input type="submit">
</form>
```
